"use client";
/* Scout — shared icons + UI primitives (ported from the design prototype) */
...
const ICON_PATHS: Record<string, React.ReactNode> = {
  ...
  save: <><path d="M5 3h11l3 3v15H5V3Z" /><path d="M8 3v6h7M8 21v-7h8v7" /></>,
...
type IconProps = {
  name: string; size?: number; stroke?: number; fill?: boolean;
  style?: React.CSSProperties;
} & Omit<React.SVGProps<SVGSVGElement>, "fill" | "stroke">;

export function Icon({ name, size = 18, stroke = 1.7, fill = false, style, ...rest }: IconProps) {
...
/* ── Button ────────────────────────────────────────────────────── */
type BtnProps = {
  variant?: "primary" | "soft" | "ghost" | "outline" | "quiet";
  size?: "sm" | "md" | "lg";
  icon?: string; iconRight?: string;
  children?: React.ReactNode; style?: React.CSSProperties;
} & React.ButtonHTMLAttributes<HTMLButtonElement>;

export function Button({ variant = "primary", size = "md", icon, iconRight, children, style, ...rest }: BtnProps) {
  const sizes = {
...
type TagProps = {
  ...
  icon?: string; children?: React.ReactNode; style?: React.CSSProperties;
};
export function Tag({ tone = "neutral", icon, children, style }: TagProps) {
  const tones = {
...
export function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { tone: TagProps["tone"]; label: string; dot: string }> = {
...
export function Select({ value, onChange, options, style }: { value: string; onChange: (v: string) => void; options: Opt[]; style?: React.CSSProperties }) {
  return (
...
} & Omit<React.InputHTMLAttributes<HTMLInputElement>, "onChange" | "value" | "style">;
export function TextInput({ value, onChange, placeholder, icon, style, ...rest }: TextInputProps) {
...
export function ScoreRing({ score, size = 168, animate = true }: { score: number; size?: number; animate?: boolean }) {
